...
// ---- onChanged 帧增量计费累加器（持久化分段账本）----
// 订阅 sessionProjections.onChanged：每次 contextVistaModelUsage 变化，计算
// delta（当前累计 - 上次累计），用「该事件发生时刻(event.time)」的峰谷与模型
// 价格计费，累加进归属会话（子代理按 header.parentSession 归到主对话）。
//
// 计费是写穿式账本：每帧/清空后落盘到 ~/.dsh/storages/context-vista-billing.json，
// 重启后 load() 续账。current 拆成 self / subagents 两段，清空时整段归档进
// segments（固定位置），「对话总费用」= Σ segments + self + subagents。
//
...
function defaultBillingFilePath() {
  const home = process.env.DSH_HOME || join(homedir(), '.dsh')
  return join(home, 'storages', 'context-vista-billing.json')
}

function createCostAccumulator(getPricing, getRoutes, billingFile) {
  const lastByModel = new Map() // sessionId -> byModel（累计快照）
  const costByOwner = new Map() // ownerSessionId -> { self, subagents, segments }
  ...
  // 一段计费范围（self 或 subagents）：费用按模型 + 总量 + 四字段 tokens。
  function emptyScope() {
    return { byModel: {}, total: 0, tokens: emptyBucketSet() }
  }

  function entryFor(ownerId) {
    let e = costByOwner.get(ownerId)
    if (!e) {
      e = { self: emptyScope(), subagents: emptyScope(), segments: [] }
  ...
  // 事件时间锚：按「该事件发生时刻」的峰谷计价，而非回调到达时刻。主会话
  // baseSeq 通常为 0，events[seq] 即该事件；fork/replay（baseSeq>0）时线性回退。
  function eventTimeOf(session, seq) {
    try {
      const events = session && session.events
      if (!Array.isArray(events)) return new Date()
      const direct = events[seq]
      if (direct && direct.seq === seq && direct.time != null) return new Date(direct.time)
      for (let i = 0; i < events.length; i++) {
        if (events[i] && events[i].seq === seq && events[i].time != null) return new Date(events[i].time)
      }
    } catch {}
    return new Date()
  }

  // 确定性回填：把事件数组逐帧按 event.time 计费，返回 { byModel(费用), tokens }。
  // 仅用于首次观察某会话时补齐部署前的历史费用（与投影 apply 共享同一 token 口径）。
  function foldOwnCost(events) {
    const pricing = getPricing()
    const routes = getRoutes()
    const costByModel = {}
    const tokens = { byModel: {}, totals: zeroBuckets() }
    for (const event of events || []) {
      if (!event || event.type !== 'assistant/message') continue
      const usage = event.data && event.data.usage
      const source = event.data && event.data.message && event.data.message.source
      if (!usage || !source || !source.model) continue
      const provider = typeof source.provider === 'string' && source.provider !== '' ? source.provider : 'default'
      const modelKey = provider + ':' + source.model
      const delta = {
        uncachedInputTokens: usage.inputTokens || 0,
      ...
      tokens.byModel[modelKey] = addBuckets(tokens.byModel[modelKey] || zeroBuckets(), delta)
      tokens.totals = addBuckets(tokens.totals, delta)
      const now = event.time != null ? new Date(event.time) : new Date()
      const cost = bucketCost(delta, modelKey, pricing.pricing, routes, pricing, now)
      if (cost !== undefined && cost > 0) costByModel[modelKey] = (costByModel[modelKey] || 0) + cost
    }
    return { byModel: costByModel, tokens }
  }
  ...
  function applyFrame(session, value, seq) {
    ...
    const entry = entryFor(ownerId)
    const scopeKey = isSub ? 'subagents' : 'self'

    // 首次观察该会话：用确定性回填初始化当前段（覆盖任何旧值），补齐部署前历史；
    // lastByModel 持久化后，重启会命中 had=true，不再重复回填。
    const had = lastByModel.has(session.id)
    if (!had) {
      const history = foldOwnCost(session.events ? session.events.slice(0, seq) : [])
      let total = 0
      for (const k of Object.keys(history.byModel)) total += history.byModel[k] || 0
      entry[scopeKey] = { byModel: history.byModel, total, tokens: history.tokens }
    }
    const target = entry[scopeKey]
    ...
    const pricing = getPricing()
    const routes = getRoutes()
    const now = eventTimeOf(session, seq)
    let dirty = false
    for (const modelKey of Object.keys(byModel)) {
  ...
  // ---- 分段归档 + 对话总费用 ----
  // 清空当前段：把 self/subagents 深拷贝进 segments（固定位置），再清零重开一段。
  function clear(ownerId) {
    const entry = costByOwner.get(ownerId)
    if (!entry) return false
    const selfTotal = entry.self ? entry.self.total : 0
    const subTotal = entry.subagents ? entry.subagents.total : 0
    if (selfTotal > 0 || subTotal > 0) {
      entry.segments.push({
        id: randomUUID(),
        clearedAt: Date.now(),
        self: entry.self,
        subagents: entry.subagents,
        total: selfTotal + subTotal,
      })
    }
    entry.self = emptyScope()
    entry.subagents = emptyScope()
    touchSnapshot()
    schedulePersist()
    return true
  }
  ...
  function totalOf(ownerId) {
    const entry = costByOwner.get(ownerId)
    if (!entry) return 0
    let total = (entry.self ? entry.self.total : 0) + (entry.subagents ? entry.subagents.total : 0)
    for (const seg of entry.segments || []) total += isNum(seg && seg.total) ? seg.total : 0
    return total
  }

  // ---- 快照（供投影下发到客户端）----
  // 返回 { ownerId: {self, subagents, segments} } 的稳定引用；仅在聚合变化时
  // 重建，投影 apply 用 Object.is 检测引用变化以触发帧推送。entry 是活对象，
  // 快照里引用同一 entry，序列化到客户端时即当前值。
  ...
  // ---- 持久化（写穿到 ~/.dsh/storages，重启后续账）----
  const persistPath = billingFile || defaultBillingFilePath()
  let persistTimer = null
  let persistDirty = false

  // 从磁盘恢复一段 scope；形状损坏时回退到空 scope，绝不抛错。
  function normalizeScope(raw) {
    const scope = raw && typeof raw === 'object' ? raw : {}
    const tokens = scope.tokens && typeof scope.tokens === 'object' ? scope.tokens : {}
    return {
      byModel: scope.byModel && typeof scope.byModel === 'object' ? scope.byModel : {},
      total: isNum(scope.total) ? scope.total : 0,
      tokens: {
        byModel: tokens.byModel && typeof tokens.byModel === 'object' ? tokens.byModel : {},
        totals: tokens.totals && typeof tokens.totals === 'object' ? tokens.totals : zeroBuckets(),
      },
    }
  }

  // 从磁盘恢复一条 owner 记录（self/subagents/segments），形状损坏时回退。
  function normalizeEntry(rec) {
    return {
      self: normalizeScope(rec && rec.self),
      subagents: normalizeScope(rec && rec.subagents),
      segments: Array.isArray(rec && rec.segments) ? rec.segments.filter((s) => s && typeof s === 'object') : [],
    }
  }
  ...
  function load() {
    try {
      ...
      if (parsed.v === 2) {
        const lm = parsed.lastByModel
        if (lm && typeof lm === 'object') {
          for (const [id, byModel] of Object.entries(lm)) {
            if (byModel && typeof byModel === 'object') lastByModel.set(id, byModel)
          }
        }
        const owners = parsed.owners
        if (owners && typeof owners === 'object') {
          for (const [id, rec] of Object.entries(owners)) {
            if (rec && typeof rec === 'object') costByOwner.set(id, normalizeEntry(rec))
          }
        }
      } else {
        // v1 旧格式（扁平 {ownerId: {...}}）：只回填 owners，lastByModel 留空，
        // 首次观察会确定性回填补齐历史（旧数据极小，可被覆盖）。
        for (const [id, rec] of Object.entries(parsed)) {
          if (!rec || typeof rec !== 'object' || id === 'v' || id === 'lastByModel' || id === 'owners') continue
          costByOwner.set(id, normalizeEntry(rec))
        }
      }
    } catch (err) {
      // 文件缺失/损坏时从空账本开始，估算数据绝不阻断插件。
    }
  }

  // 原子写盘：临时文件 + rename，避免读到半截文件。失败仅静默。
  function flushPersist() {
    ...
    try {
      const obj = {
        v: 2,
        lastByModel: Object.fromEntries(lastByModel),
        owners: {},
      }
      for (const [id, entry] of costByOwner.entries()) obj.owners[id] = entry
...
const I18N = {
  ...
  en: {
    ...
    billingCleared: 'Billing cleared; previous cost archived',
  },
}
...
export function apply(ctx, config = {}) {
  ...
  // ---- 帧增量计费累加器（onChanged，持久化分段账本）----
  // 订阅投影变化，每次 contextVistaModelUsage 变化时用「帧增量 × 事件时间峰谷」计费，
  // 子代理按 header.parentSession 归到主对话；卸载时清定时器并最终落盘收尾。
  const accumulator = createCostAccumulator(() => pricing, () => routes)
  ctx.effect(() => () => accumulator.dispose())
  ...
  ctx.effect(() => ctx.commands.register({
    ...
    handler: async (invocation) => {
      ...
      // 对话总费用 = 已归档各段之和 + 当前段（self + subagents）。
      const conversationTotal = accumulator.totalOf(session.id)
      ...
      if (conversationTotal > 0) {
        lines.push(t(lang, 'conversationTotal') + ': ' + fmtCost(conversationTotal, currency))
      }
  ...
  // 清空当前会话计费：把当前段归档进 segments 后清零，开始新一段。
  ctx.effect(() => ctx.commands.register({
    name: 'context-clear',
    description: 'Archive and reset the current session billing',
    handler: async (invocation) => {
      try {
        const session = invocation.agent.session
        accumulator.clear(session.id)
        return { kind: 'success', text: t(resolveLocale(ctx), 'billingCleared') }
      } catch (err) {
        return { kind: 'error', text: 'context-vista: ' + (err && err.message ? err.message : String(err)) }
      }
    },
