...
window.__ModuleLoader__.load({
  ...
  factory: function (require) {
    ...
    var zh = {
      ...
      'compact': '压缩上下文',
      'compacting': '压缩中…',
      'compactHint': '执行 /compact，让模型压缩会话上下文',
    };
    var en = {
      'title': 'Context usage',
      ...
      'includingSubagents': 'Incl. subagents',
      'conversationTotal': 'Conversation total',
      'clearBilling': 'Clear billing',
      'peak': 'Peak',
      ...
      'compactHint': 'Run /compact to compress session context',
    };
    ...
    var css = [
      ...
      '.dcc-hud-compact{box-sizing:border-box;width:100%;border:1px solid var(--dsw-alias-border-l2);background:transparent;color:var(--dsw-alias-label-primary);border-radius:8px;padding:6px 8px;font:inherit;font-size:12px;line-height:16px;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:6px;margin-top:2px}',
      '.dcc-hud-compact:hover:not(:disabled){background:var(--dsw-alias-interactive-bg-hover)}',
      '.dcc-hud-compact:disabled{opacity:.5;cursor:default}',
      '.dcc-hud-clear{box-sizing:border-box;width:100%;border:1px solid var(--dsw-alias-border-l2);background:transparent;color:var(--dsw-alias-label-secondary);border-radius:8px;padding:5px 8px;font:inherit;font-size:11px;line-height:16px;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:6px;margin-top:2px}',
      '.dcc-hud-clear:hover:not(:disabled){background:var(--dsw-alias-interactive-bg-hover);color:var(--dsw-alias-label-primary)}',
      '.dcc-hud-clear:disabled{opacity:.5;cursor:default}'
    ].join('\n');
    ...
    function resolveRouteConfig(pricing, routes, provider, defaults) {
      ...
      for (var i = 0; i < keys.length; i++) {
        var entry = pricing ? pricing[keys[i]] : undefined;
    ...
    function summarizeByModel(byModel) {
      ...
      Object.keys(byModel).forEach(function (modelKey) {
        var sep = modelKey.indexOf(':');
        var provider = sep >= 0 ? modelKey.slice(0, sep) : modelKey;
        var model = sep >= 0 ? modelKey.slice(sep + 1) : modelKey;
        rows.push({ provider: provider, model: model, totals: byModel[modelKey] || zeroBuckets() });
      });
      return rows;
    }
    ...
    function fmtCost(value, currency) {
      if (!isNum(value)) return '–';
      var unit = typeof currency === 'string' && currency !== '' ? currency : DEFAULT_CURRENCY;
    ...
    // Track the /compact command's lifecycle from the transcript. A command
    // node is admitted as soon as `session.command()` reaches the host (the
    // `command/run` event lands immediately), so its mere presence is NOT a
    // completion signal — that was the "flash and clear" bug. Settlement is:
    //   - the newest command node's `outcome` flips from null to non-null
    //     (the `command/done` payload lands), OR
    //   - a `compaction` checkpoint node appears (successful compaction).
    // The `running` bit is useless here: a host-side compaction is not a model
    // turn, so it never toggles that bit.
    function compactLifecycle(snapshot) {
      var compactions = 0;
      var lastSeq = null;
      ...
      return { compactions: compactions, lastSeq: lastSeq, lastSettled: lastSettled };
    }
    ...
    function ContextCommandView(props) {
      ...
      var modelUsage = useProjection('contextVistaModelUsage');
      var pricingProj = useProjection('contextPricing');
      // 子代理聚合（tokens + 费用）来自宿主 onChanged 累加器的快照投影，按会话 id 查。
      var subagentProj = useProjection('contextVistaSubagent');
      var sessionId = props.sessionId;
      var subEntry = subagentProj && sessionId ? subagentProj[sessionId] : undefined;
      var selfEntry = subEntry && subEntry.self;
      var sub = subEntry && subEntry.subagents;
      var subTokens = sub && sub.tokens && sub.tokens.totals;
      var subCost = sub && isNum(sub.total) ? sub.total : undefined;
      var hasSubTokens = !!(subTokens && ((subTokens.uncachedInputTokens || 0) > 0 || (subTokens.outputTokens || 0) > 0 || (subTokens.cacheReadTokens || 0) > 0 || (subTokens.cacheWriteTokens || 0) > 0));
      var hasSub = hasSubTokens || subCost !== undefined;
      ...
      var routeCfg = resolveRouteConfig(cfg.pricing, cfg.routes, provider, cfg);
      // 费用读宿主按帧累加的账本（self 段），不再用「全量 tokens × 当前峰谷」重算。
      var cost = (selfEntry && isNum(selfEntry.total) && selfEntry.total > 0) ? selfEntry.total : undefined;
      var modelRows = summarizeByModel(modelUsage && modelUsage.byModel);
      modelRows.forEach(function (row) {
        var key = row.provider + ':' + row.model;
        var m = (selfEntry && selfEntry.byModel && isNum(selfEntry.byModel[key]) && selfEntry.byModel[key] > 0) ? selfEntry.byModel[key] : undefined;
        row.cost = m;
      });
      // 对话总费用 = 已归档各段之和 + 当前段（self + subagents）。
      var segTotal = 0;
      if (subEntry && Array.isArray(subEntry.segments)) {
        subEntry.segments.forEach(function (s) { if (isNum(s && s.total)) segTotal += s.total; });
      }
      var convTotal = (selfEntry && isNum(selfEntry.total) ? selfEntry.total : 0) + (sub && isNum(sub.total) ? sub.total : 0) + segTotal;
      ...
      if (usage || modelUsage) {
        children.push(React.createElement('div', { key: 'totals' },
          ...
          React.createElement('div', { className: 'dcc-totals' },
            totalRow(t('input'), usage && usage.uncachedInputTokens),
            totalRow(t('output'), usage && usage.outputTokens),
            totalRow(t('cacheRead'), usage && usage.cacheReadTokens),
            totalRow(t('cacheWrite'), usage && usage.cacheWriteTokens)
          ),
          cost !== undefined ? React.createElement('div', { className: 'dcc-cost' },
            React.createElement('span', { className: 'dcc-cost-label' }, t('cost')),
            React.createElement('span', { className: 'dcc-cost-value' }, fmtCost(cost, routeCfg.currency))
          ) : null,
          cost !== undefined ? React.createElement('div', { className: 'dcc-cost-note' },
            t('estimateNote') + ' · ' + t('modelBreakdown')
          ) : null,
          modelRows.length > 0 ? React.createElement('div', { className: 'dcc-model' },
            React.createElement('div', { className: 'dcc-section-label' }, t('modelBreakdown')),
            modelRows.map(function (row) {
              return React.createElement('div', { className: 'dcc-total', key: row.provider + ':' + row.model },
                React.createElement('span', { className: 'dcc-total-label' }, row.model + ' @ ' + row.provider),
                React.createElement('span', { className: 'dcc-total-value' },
                  compact(row.totals.uncachedInputTokens) + ' / ' + compact(row.totals.outputTokens) +
                  (row.cost !== undefined ? ' · ' + fmtCost(row.cost, routeCfg.currency) : '')
                )
              );
            })
          ) : null
      ...
      if (hasSub) {
        children.push(React.createElement('div', { key: 'subagents' },
          React.createElement('div', { className: 'dcc-section-label' }, t('subagentUsage')),
          hasSubTokens ? React.createElement('div', { className: 'dcc-totals' },
            totalRow(t('input'), subTokens.uncachedInputTokens),
            totalRow(t('output'), subTokens.outputTokens),
            totalRow(t('cacheRead'), subTokens.cacheReadTokens),
            totalRow(t('cacheWrite'), subTokens.cacheWriteTokens)
          ) : null,
          subCost !== undefined ? React.createElement('div', { className: 'dcc-cost' },
            React.createElement('span', { className: 'dcc-cost-label' }, t('subagentCost')),
            React.createElement('span', { className: 'dcc-cost-value' }, fmtCost(subCost, routeCfg.currency))
          ) : null
        ));
      }

      if (convTotal > 0) {
        children.push(React.createElement('div', { key: 'conversation-total', className: 'dcc-cost' },
          React.createElement('span', { className: 'dcc-cost-label' }, t('conversationTotal')),
          React.createElement('span', { className: 'dcc-cost-value' }, fmtCost(convTotal, routeCfg.currency))
        ));
      }
    ...
    function useHudData(currentId) {
      ...
      React.useEffect(function () {
        ...
        // 防御：投影 face 缺失/抛错时返回 undefined，避免 HUD 渲染崩溃。
        function snap(proj) {
          try {
            var f = session.projections.faceOf(proj);
            return f && typeof f.getSnapshot === 'function' ? f.getSnapshot() : undefined;
          } catch (err) { return undefined; }
        }
        function read() {
          var snapState;
          try { snapState = session.getSnapshot(); } catch (err) { snapState = undefined; }
          setData({
            pressure: snap('contextPressure'),
            breakdown: snap('contextBreakdown'),
            usage: snap('tokenUsage'),
            modelUsage: snap('contextVistaModelUsage'),
            pricing: snap('contextPricing'),
            subagent: snap('contextVistaSubagent'),
            snapshot: snapState,
          });
        }
        read();
        var off = [];
        try { off.push(session.subscribe(read)); } catch (err) {}
        ['contextPressure', 'contextBreakdown', 'tokenUsage', 'contextVistaModelUsage', 'contextPricing', 'contextVistaSubagent'].forEach(function (proj) {
          try {
            var f = session.projections.faceOf(proj);
            if (f && typeof f.subscribe === 'function') off.push(f.subscribe(read));
          } catch (err) {}
        });
        return function () { for (var i = 0; i < off.length; i++) { try { off[i](); } catch (err) {} } };
      }, [currentId]);
      return state[0];
    }
    ...
    function ContextHud(props) {
      ...
      // ---- /compact trigger ----
      // The Session object behind the current binding exposes command(line), the
      // dedicated slash-command admission surface (the composer routes "/"-prefixed
      // input through it). Sending "/compact" reaches the built-in command-compact
      // plugin on the host — unlike prompt(), which would send it as a plain message.
      var running = !!(data && data.snapshot && data.snapshot.running);
      var sessionBinding = SESSIONS && currentId ? SESSIONS.binding(currentId) : undefined;
      var session = sessionBinding && sessionBinding.session;
      var compactState = React.useState(false);
      var compacting = compactState[0];
      var setCompacting = compactState[1];
      var compactingRef = React.useRef(false);
      var compactStartRef = React.useRef(null); // lifecycle counts captured at click
      var compactTimerRef = React.useRef(null);
      var clearState = React.useState(false);
      var clearing = clearState[0];
      var setClearing = clearState[1];
      var clearTimerRef = React.useRef(null);

      function stopCompacting() {
        compactingRef.current = false;
        compactStartRef.current = null;
      ...
      // Completion is detected three ways (see compactLifecycle):
      //   1. the newest command node's `outcome` becomes non-null (settled);
      //   2. a `compaction` checkpoint node appears (successful compaction);
      //   3. a 30s timeout backstops everything else (no done / transport hang).
      var lifecycle = data && data.snapshot ? compactLifecycle(data.snapshot) : { compactions: 0, lastSeq: null, lastSettled: false };
      React.useEffect(function () {
        if (!compactingRef.current) return;
        var start = compactStartRef.current;
        if (!start) return;
        var newCommand = lifecycle.lastSeq != null && (start.lastSeq == null || lifecycle.lastSeq > start.lastSeq);
        var settled = newCommand && lifecycle.lastSettled;
        if (settled || lifecycle.compactions > start.compactions) {
          stopCompacting();
        }
      });

      React.useEffect(function () {
        if (!running && compactingRef.current) stopCompacting();
      }, [running]);

      React.useEffect(function () {
        return function () {
          if (compactTimerRef.current != null) clearTimeout(compactTimerRef.current);
      ...
      var modelUsage = data.modelUsage;
      var pricingProj = data.pricing;
      var route = currentRoute(data.snapshot);
      var provider = route && route.provider;
      var cfg = pricingProj && (pricingProj.timezone !== undefined || pricingProj.pricing) ? pricingProj : BUILTIN_CONFIG;
      var routeCfg = resolveRouteConfig(cfg.pricing, cfg.routes, provider, cfg);
      // 子代理聚合（live）：含子代理总费用 = 当前段 self + 当前段子代理。
      var subAgg = data.subagent;
      var subEntry = subAgg && currentId ? subAgg[currentId] : undefined;
      var selfEntry = subEntry && subEntry.self;
      var subTotal = subEntry && subEntry.subagents && isNum(subEntry.subagents.total) ? subEntry.subagents.total : 0;
      // 费用读宿主按帧累加的账本（self 段），不再用「全量 tokens × 当前峰谷」重算。
      var cost = (selfEntry && isNum(selfEntry.total) && selfEntry.total > 0) ? selfEntry.total : undefined;
      // 对话总费用 = 已归档各段之和 + 当前段（self + subagents）。
      var segTotal = 0;
      ...
      var convTotal = (selfEntry && isNum(selfEntry.total) ? selfEntry.total : 0) + subTotal + segTotal;
      // 主要（最贵）模型，供 HUD note 展示。
      var modelRows = summarizeByModel(modelUsage && modelUsage.byModel);
      var dominantModel = null;
      modelRows.forEach(function (row) {
        var key = row.provider + ':' + row.model;
        var c = (selfEntry && selfEntry.byModel && isNum(selfEntry.byModel[key])) ? selfEntry.byModel[key] : undefined;
        row.cost = c;
        if (c !== undefined && (dominantModel === null || c > dominantModel.cost)) dominantModel = row;
      });
      ...
      var onCompact = function () {
        if (!session || typeof session.command !== 'function' || running || compactingRef.current) return;
        compactingRef.current = true;
        compactStartRef.current = lifecycle;
        setCompacting(true);
        if (compactTimerRef.current != null) clearTimeout(compactTimerRef.current);
        compactTimerRef.current = setTimeout(stopCompacting, 30000);
        Promise.resolve(session.command('/compact'))
          .then(function (result) {
            if (!result || result.ok === false || !result.value || result.value.matched === false) {
              stopCompacting();
            }
          })
          .catch(function () {
      ...
      // ---- 清空计费：归档当前段后清零（走 /context-clear 命令）----
      var onClear = function () {
        if (!session || typeof session.command !== 'function' || clearing) return;
        setClearing(true);
        if (clearTimerRef.current != null) clearTimeout(clearTimerRef.current);
        clearTimerRef.current = setTimeout(function () { setClearing(false); }, 5000);
        Promise.resolve(session.command('/context-clear'))
          .catch(function () {})
          .then(function () {
      ...
      if (expanded) {
        ...
        body.push(React.createElement('div', { className: 'dcc-hud-foot' },
          ...
          cost !== undefined ? React.createElement('div', { className: 'dcc-hud-row' },
            ...
            React.createElement('span', { className: 'dcc-hud-row-v dcc-hud-cost' }, fmtCost(cost, routeCfg.currency))
          ) : null,
          subTotal > 0 ? React.createElement('div', { className: 'dcc-hud-row' },
            React.createElement('span', { className: 'dcc-hud-row-l' }, t('includingSubagents')),
            React.createElement('span', { className: 'dcc-hud-row-v dcc-hud-cost' }, fmtCost((cost !== undefined ? cost : 0) + subTotal, routeCfg.currency))
          ) : null,
          convTotal > 0 ? React.createElement('div', { className: 'dcc-hud-row' },
            React.createElement('span', { className: 'dcc-hud-row-l' }, t('conversationTotal')),
            React.createElement('span', { className: 'dcc-hud-row-v dcc-hud-cost' }, fmtCost(convTotal, routeCfg.currency))
          ) : null,
          cost !== undefined ? React.createElement('div', { className: 'dcc-hud-note' },
            (dominantModel ? (dominantModel.model + (dominantModel.provider ? ' @ ' + dominantModel.provider : '')) : t('modelBreakdown'))
          ) : null
        ));
        body.push(React.createElement('button', {
          type: 'button',
          className: 'dcc-hud-compact',
          disabled: !session || typeof session.command !== 'function' || running || compacting,
          title: t('compactHint'),
          onClick: onCompact
        }, compacting ? t('compacting') : t('compact')));
        body.push(React.createElement('button', {
          type: 'button',
          className: 'dcc-hud-clear',
          disabled: !session || typeof session.command !== 'function' || clearing,
          title: t('clearBilling'),
          onClick: onClear
        }, clearing ? '…' : t('clearBilling')));
      }
