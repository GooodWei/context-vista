...
window.__ModuleLoader__.load({
  ...
  factory: function (require) {
    ...
    function resolveModelEntry(routeConfig, model) {
      var models = (routeConfig && routeConfig.models) || {};
    ...
    // 按模型分别计价后累加：每个模型用自己的价格 × 当前时刻峰谷。
    // 与 host 的帧增量计费口径一致——时间锚点是「当前时刻」，不再按分钟分桶。
    function estimateCostByModel(byModel, pricing, routes, defaults, now) {
      if (!byModel) return undefined;
      var total = 0;
      var hasAny = false;
      var at = now || new Date();
      Object.keys(byModel).forEach(function (modelKey) {
        var buckets = byModel[modelKey] || zeroBuckets();
        var sep = modelKey.indexOf(':');
        var provider = sep >= 0 ? modelKey.slice(0, sep) : modelKey;
        var model = sep >= 0 ? modelKey.slice(sep + 1) : modelKey;
        var routeCfg = resolveRouteConfig(pricing, routes, provider, defaults);
        var peak = isPeak(at, routeCfg.timezone, routeCfg.peakWindows);
        var price = periodPrice(resolveModelEntry(routeCfg, model), peak);
        var cost = estimateCost(buckets, price);
        if (cost !== undefined) { total += cost; hasAny = true; }
      });
    ...
    function ContextCommandView(props) {
      ...
      // 按「模型 × 分钟」逐个计价后累加（不再用单一峰谷价套全量）。
      var cost = estimateCostByModel(modelUsage && modelUsage.byModel, cfg.pricing, cfg.routes, cfg);
      var modelRows = summarizeByModel(modelUsage && modelUsage.byModel);
      modelRows.forEach(function (row) {
        var single = {};
        single[row.provider + ':' + row.model] = (modelUsage && modelUsage.byModel && modelUsage.byModel[row.provider + ':' + row.model]) || {};
        row.cost = estimateCostByModel(single, cfg.pricing, cfg.routes, cfg);
      });
    ...
    function ContextHud(props) {
      ...
      // 按「模型 × 分钟」逐个计价后累加。
      var cost = estimateCostByModel(modelUsage && modelUsage.byModel, cfg.pricing, cfg.routes, cfg);
      // 子代理聚合（live）：含子代理总费用 = 当前会话全量费用 + 子代理实时费用。
      ...
      modelRows.forEach(function (row) {
        ...
        single[row.provider + ':' + row.model] = (modelUsage && modelUsage.byModel && modelUsage.byModel[row.provider + ':' + row.model]) || {};
        row.cost = estimateCostByModel(single, cfg.pricing, cfg.routes, cfg);
        if (row.cost !== undefined && (dominantModel === null || row.cost > dominantModel.cost)) dominantModel = row;
      });
